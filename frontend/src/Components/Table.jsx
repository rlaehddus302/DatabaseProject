                        ...
                        <tr>
                            <th className='p-0'>
                                <div>
                                    {time.map((value) => {
                                        return(
                                            <div className={`${styles.cell} align-middle`}>
                                                {value}
                                            </div>
                                        )
                                    })}
                                        ...
                                        })}
                                        {
                                            prop.prop.map((prevStat, index)=>{
                                                ...
                                                .map(value => {
                                                    ...
                                                    let top = ((startInt-8)+startFloat*10/6)*40
                                                    let locations = value.location;
                                                    let location = locations.split("(")
                                                    if(week[rowIndex] === value.week)
                                                    {
                                                        return(
                                                            <button type='button' className={`${session[index+1]} w-100 text-center overflow-hidden`} style={{position:'absolute',top: `${top+1}px`, height:`${height-1}px`, fontSize: "12px" }} data-bs-html="true" data-bs-toggle="popover" 
